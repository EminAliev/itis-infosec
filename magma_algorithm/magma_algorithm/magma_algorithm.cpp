...
void printman() {
    cout << "magma_algorithm [-h|--help]" << endl;
    cout << "magma_algorithm [--ecb|--ctr|--ofb|--cbc|--cfb] {-e|-d} -k <key file> [operations]" << endl;
    cout << "magma_algoritm --mac -k <key file> [operations]" << endl;
    cout << "• -e – шифрование." << endl;
    cout << "• -d – расшифрование." << endl;
    cout << "• -k – файл с бинарным ключом." << endl;
...
void readkey(uint32_t KEY[32], char* name) {
    ...
    if (!keyd.is_open()) {
        cerr << "Невозможно открыть файл " << name << endl;
        printman();
        exit(3);
    };
    for (int i = 0; i < 8; i++) {
    ...
    while (i < size) {
        ...
        if (enc) {
            ...
            if (i == size - 1) {
                ...
                if (size1 == -1) {
                    cerr << "Плохой текст для шифрования" << endl;
                    printman();
                    exit(6);
                }
                else {
...
void cbc(bool enc, uint32_t KEY[32], char* ivname, int size) {
    ...
    while (i < size) {
        ...
        if (!enc && i == size - 1) {
            int size1 = checkprint64(c);
            if (size1 == -1) {
                cerr << "Плохой текст для расшифровки" << endl;
                printman();
                exit(6);
            }
            else {
...
void checkiv32(char* name) {
    ...
    file.close();
    if (size != 4) {
        cerr << "Не 4-х байтовый ключ" << endl;
        exit(10);
    }
}
...
void setflags(int argc, char* argv[]) {
    ...
    while (i < argc) {
        if (!strcmp(argv[i], "--ecb")) {
            ...
            i++;
            if (i == argc)
                throw "Нет файла после -v";
            vname = argv[i];
            i++;
        }
        else {
            cerr << "Недопустимый параметр " << argv[i] << endl;
            printman();
            exit(6);
        }
    }
...
void checkcorrect() {
    ...
    if (fmac) {
        if (e)
            throw "Недопустимое использование -e с --mac";
        else if (d)
            throw "Недопустимое использование -d с --mac";
        else if (v)
            throw "UНедопустимое использование -v с --mac";
    }
    if (fecb && v)
        throw "Недопустимое использование -v с --ecb";
    if (e && d)
        throw "Недопустимое использование -e и -d";
    if (!fmac && !e && !d)
        throw "Недопустимое использование -e или -d";
    if (!k)
        throw "Недопустимое использование -k <keyfile>";
}
...
void callmode(uint32_t KEY[32]) {
    ...
    if (fecb) {
        ...
        cerr << "Недопустимое использование" << endl;
        exit(4);
    }
    if (cr)
...
void inout() {
    ...
    if (in) {
        if (!(f = freopen(iname, "rb", stdin))) {
            cerr << "Неправильный формат файла" << endl;
            exit(1);
        };
    }
    else {
